...
首先是重复动作的分段。比如说深蹲，深蹲从站立、下蹲到最低点、再次站起的整个过程中，膝角近似从$180 degree$到$45 degree$再回到$180 degree$。理论上这个过程是连续的，所以它就像一个波的正半部分一样。

抽象出来说，我们面临的问题是：给定一个函数$f(x) >= 0$，其有若干段波的正半部分组成，每段波之间存在间隔，间隔基本是一段常数，每段波的波长不定。要求精确地分割出每一段波，即找出每一段的起始和结束点。
