...
def _build_pro_llm(temperature: float = MAX_TEMPERATURE) -> ChatDeepSeek:
    return ChatDeepSeek(
...
def _build_light_llm(temperature: float = MAX_TEMPERATURE) -> ChatDeepSeek:
    return ChatDeepSeek(
        api_key=DEEPSEEK_API_KEY,
        api_base=DEEPSEEK_BASE_URL,
...
def create_refiner_llm() -> ChatDeepSeek:
    return _build_light_llm(temperature=0.2)


def create_generator_agent():
    llm = _build_pro_llm(temperature=0.1)
...
def create_reviewer_llm() -> ChatDeepSeek:
    return _build_light_llm(temperature=0.0)


def create_summarizer_llm() -> ChatDeepSeek:
    return _build_light_llm(temperature=0.1)
